...
export default (controller) => {
  // 서장님 찾기
  controller.hears([new RegExp(`(${yorks.join('|')})`, 'g')], [
    'direct_message',
  ], (bot, message) => {
  ...
  // 힐 안 주는 메르시
  controller.hears('힐', ['direct_message'], (bot, message) => {
    bot.reply(message, {
      username: '메르시',
      attachments: [{
        title: 'ㄴㄴ',
  ...
  // 갓스터
  controller.hears(['갓스터', '함스터갓'], ['direct_message'], (bot, message) => {
    bot.reply(message, 'https://www.youtube.com/watch?v=LA4CTzhrLu8');
  });

  controller.hears(['버피', 'Buffy', 'buffy'], ['direct_message'], (bot, message) => {
    bot.reply(message, {
      username: '버피 홍보대사',
      attachments: [
        {
